...
def split_on_time_and_speed(df, max_gap="2h", max_speed=12):
    df = df.copy()
    df["date_time_utc"] = pd.to_datetime(df["date_time_utc"])
    df = df.sort_values(["mmsi", "date_time_utc"])
    ...
    df["segment_id"] = break_edge.groupby(df["mmsi"]).cumsum()
...
def reconnect_subtracks(df, max_gap="10min", max_speed=15):
    df = df.copy()
    df["date_time_utc"] = pd.to_datetime(df["date_time_utc"])
    df = df.sort_values(["mmsi", "date_time_utc"])
...
if __name__ == "__main__":
    ...
    df_small.to_parquet("Processed_AIS_2024/Cleaned/01_new_order.parquet", index=False)
